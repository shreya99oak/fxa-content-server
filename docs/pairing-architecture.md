...
The Channel Server will now relay websocket messages from Mobile to Desktop and vice-versa. The supplicant broker handles the
...
The pairing flow is made possible by messages are forwarded between the Desktop native code and the web content page opened on the supplicant device.
...
* The Native Desktop code takes care of the following:
